...
DWORD g_pdbAge = -1;
DWORD g_pdbAgeAddr = 0;
GUID g_pdbSig = {0, 0, 0, 0};
DWORD g_pdbSigAddr = 0;
DWORD g_pdbAge2 = -1;
DWORD g_pdbAge2Addr = 0;
...
void printPdbStream(MsfFile& msf, std::ostream& os) {
    ...
    os << std::endl;
    g_pdbAge = header.age;
    g_pdbAgeAddr = pagePos + (long long)&header.age - (long long)&header;
    memcpy(&g_pdbSig, header.sig70, sizeof(g_pdbSig));
    g_pdbSigAddr = pagePos + (long long)&header.sig70 - (long long)&header;
}
...
void printDbiStream(MsfFile& msf, std::ostream& os) {
    ...
    os << "Signature:                          0x" << std::hex << dbi.signature
        ...
        << std::endl;
    g_pdbAge2 = dbi.age;
    g_pdbAge2Addr = pagePos + (long long)&dbi.age - (long long)&dbi;
}
...
bool DumpPdbFile(LPCTSTR filename)
{
    _tprintf(_T("PDB File: %s \n\n"), filename);
